...
def worker(gpu, cfg):
    ...
    for step in range(resume_step, cfg.num_steps + 1): 
        ...
        ref_frame, _, video_data, captions, video_key, origin_video_data = batch
        batch_size, frames_num, _, _, _ = video_data.shape
        video_data = rearrange(video_data, 'b f c h w -> (b f) c h w')
        origin_video_data = rearrange(origin_video_data, 'b f c h w -> (b f) c h w')
        ...
        ori_video_data_list = torch.chunk(origin_video_data, origin_video_data.shape[0]//cfg.chunk_size,dim=0)
        with torch.no_grad():
            decode_data = []
            ori_decode_data = []
            ...
            origin_video_data = rearrange(origin_video_data, '(b f) c h w -> b c f h w', b = batch_size) # [B, 4, 16, 32, 56]
        ...
        # forward
        model_kwargs = {'y': y_words,  'fps': fps_tensor, 'condition_latent':origin_video_data}
        if cfg.use_fsdp:
                ...
                visual_kwards = [
                    {
                        ...
                        'condition_latent': origin_video_data
                    },
                    {
