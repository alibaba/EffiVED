...
def inference_videoedit_entrance(cfg_update,  **kwargs):
...
def get_video_data(video_url, transforms, cfg):
    print(os.path.join(cfg.origin_data_dir, video_url))
    capture = cv2.VideoCapture(os.path.join(cfg.origin_data_dir, video_url))
    frame_list = []
    while True:
    ...
    print(n_frame)
    video_data = torch.zeros(cfg.max_frames, 3,  cfg.resolution[1], cfg.resolution[0])
    frames = transforms(frame_list[:cfg.max_frames])
    video_data[:, ...] = frames
    return video_data
def worker(gpu, cfg, cfg_update):
    ...
    for idx, item in enumerate(test_list):
        ...
        ori_video_data = get_video_data(video_url, train_trans,cfg)
        #print(ori_video_data.shape)
        ori_video_data_list = torch.chunk(ori_video_data, ori_video_data.shape[0]//4,dim=0)
        with torch.no_grad():
            ori_decode_data = []
            for chunk_data in ori_video_data_list:
                latent_z = autoencoder.encode_firsr_stage(chunk_data.cuda(), cfg.scale_factor).detach()
                ori_decode_data.append(latent_z) # [B, 4, 32, 56]
            origin_video_data = torch.cat(ori_decode_data,dim=0)
            origin_video_data = rearrange(origin_video_data, '(b f) c h w -> b c f h w', b = 1) # [B, 4, 16, 32, 56]
        #print(origin_video_data.shape)
        ...
        with torch.no_grad():
            ...
            with amp.autocast(enabled=cfg.use_fp16):
                ...
                model_kwargs=[
                    {'y': y_words, 'fps': fps_tensor,'condition_latent':origin_video_data},
                    {'y': zero_y_negative, 'fps': fps_tensor,'condition_latent':origin_video_data}]
